...
const ProductsAdmin = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        ...
        console.log("Fetched products from Firebase:", data); // Debug log
        setProducts(data);
      },
      (error) => {
        console.error("Error fetching products:", error);
        alert("Məhsullar tapılmır: " + error.message);
      }
    );
    return () => unsubscribe();
  }, []);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      alert("Yükləmə xətası: " + (error.message || "Bilinməyən xəta"));
    }
  };
  ...
  const handleAddOrSaveProduct = async () => {
    ...
    if (editIndex !== null) {
      ...
      updatedProducts[editIndex] = {
        ...formProduct,
        hashtags: formProduct.hashtags
          ? formProduct.hashtags.split(",").map((tag) => tag.trim())
          : [],
      };
    } else {
      updatedProducts = [
    ...
    try {
      console.log("Saving to Firebase:", updatedProducts); // Debug log
      const productsRef = ref(db, "products");
      ...
      setEditIndex(null);
      alert(
        editIndex !== null ? "Məhsul uğurla yeniləndi" : "Məhsul uğurla əlavə olundu"
      );
    } catch (error) {
      console.error(`Error ${editIndex !== null ? "updating" : "adding"} product:`, error);
      alert(`Xəta ${editIndex !== null ? "yeniləmədə" : "əlavə etməkdə"}: ${error.message}`);
    }
  };
  ...
  const handleRemoveProduct = async (index) => {
    ...
    try {
      console.log("Removing product, saving to Firebase:", updatedProducts); // Debug log
      const productsRef = ref(db, "products");
      await set(productsRef, updatedProducts);
      setProducts(updatedProducts);
  ...
  return (
    <div className="products-admin">
      ...
      <div className="form-section">
        <h3>{editIndex !== null ? "Düzəlt" : "Əlavə et"}</h3>
        <input
          type="text"
          placeholder="Ad"
