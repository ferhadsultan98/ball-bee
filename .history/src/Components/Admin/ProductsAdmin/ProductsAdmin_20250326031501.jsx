...
const ProductsAdmin = () => {
  ...
  const [formProduct, setFormProduct] = useState({ name: '', description: '', price: '', image: '', hashtags: '' });
  ...
  const handleAddOrSaveProduct = async () => {
    if (!formProduct.name || !formProduct.description || !formProduct.price || !formProduct.image) {
    ...
    if (editIndex !== null) {
      ...
      updatedProducts[editIndex] = { ...formProduct, hashtags: formProduct.hashtags.split(',').map(tag => tag.trim()) };
    } else {
      updatedProducts = [...products, { ...formProduct, hashtags: formProduct.hashtags.split(',').map(tag => tag.trim()) }];
    }

    try {
      await set(ref(db, 'products'), updatedProducts);
      setProducts(updatedProducts);
      setFormProduct({ name: '', description: '', price: '', image: '', hashtags: '' });
      setEditIndex(null);
    } catch (error) {
      console.error(`Error ${editIndex !== null ? 'updating' : 'adding'} product:`, error);
      alert(`Failed to ${editIndex !== null ? 'update' : 'add'} product`);
    }
  };

  const handleEditProduct = (index) => {
    setEditIndex(index);
    setFormProduct({ ...products[index], hashtags: products[index].hashtags.join(', ') });
  };
  ...
  return (
    <div className="products-admin">
      ...
      <div className="form-section">
        ...
        <input
          type="text"
          placeholder="Haştaqlar vergül ilə ayrı yaz"
          value={formProduct.hashtags}
          onChange={(e) => setFormProduct((prev) => ({ ...prev, hashtags: e.target.value }))}
        />
          ...
          products.map((product, index) => (
            <div key={index} className="product-card">
              ...
              <div className="card-content">
                ...
                <p className="hashtags">Haştaqlar: {product.hashtags.join(', ')}</p>
                <div className="card-buttons">
                  <button className="edit-btn" onClick={() => handleEditProduct(index)}>
                    Sil
                  </button>
