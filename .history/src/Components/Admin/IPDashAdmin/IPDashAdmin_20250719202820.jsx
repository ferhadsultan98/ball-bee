...
import { db, ref, onValue, remove } from '../../../Firebase/Firebase';
...
const IPDashAdmin = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const ipRef = ref(db, `visitorIPs/${id}`);
      await remove(ipRef);
      setVisitorIPs(visitorIPs.filter((visitor) => visitor.id !== id));
    } catch (error) {
          ...
          <tbody>
            {paginatedIPs.map((visitor) => (
              <tr key={visitor.id}>
                ...
                <td>{truncateText(visitor.userAgent)}</td>
                <td>
                  <button
